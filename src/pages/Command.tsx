...
const Command = () => {
  ...
  const [campaignSource, setCampaignSource] = useState<CampaignSource>(
    (urlCampaignSource as CampaignSource) || "standard",
  );
  ...
  const [itemToDelete, setItemToDelete] = useState<{
    id: string;
    name: string;
    type: "mission" | "exercise" | "campaign";
  } | null>(null);
  ...
  const publicCollections =
    collections?.filter((c) => !c.is_system && c.visibility === "public" && c.created_by !== user?.id) || [];
    ...
    if (durationFilter) {
      ...
      if (durationConfig) {
        campaigns = campaigns.filter((c) => {
          const totalTime =
            c.collection_missions?.reduce((sum, cm) => {
          ...
          if (durationConfig.min && durationConfig.max)
            return totalTime >= durationConfig.min && totalTime < durationConfig.max;
          return true;
        });
      }
    }
        ...
        <div className="flex items-center justify-between gap-2 mb-4">
          ...
          <div className="flex gap-1.5 sm:gap-2 overflow-x-auto scrollbar-hide snap-x snap-mandatory -mx-1 px-1 pb-1">
            {tabs.map((tab) => {
              ...
              const tabColors = {
                campaigns: {
                  active: "bg-section-campaigns text-white border-section-campaigns",
                  inactive: "hover:border-section-campaigns/50",
                },
                missions: {
                  active: "bg-section-missions text-white border-section-missions",
                  inactive: "hover:border-section-missions/50",
                },
                exercises: {
                  active: "bg-section-command text-white border-section-command",
                  inactive: "hover:border-section-command/50",
                },
              };
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-4">
            ...
            <div className="mb-3 p-3 bg-section-missions/5 border border-section-missions/20 rounded-lg">
              <p className="text-sm text-muted-foreground leading-relaxed">
                <span className="text-section-missions font-display">MISSIONS</span> are single combat ops. Pick one.
                Execute. Get stronger.
              </p>
            </div>
            {user && (
              <SourceToggle
        ...
        {activeTab === "campaigns" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-4">
            ...
            <div className="mb-3 p-3 bg-section-campaigns/5 border border-section-campaigns/20 rounded-lg">
              <p className="text-sm text-muted-foreground leading-relaxed">
                <span className="text-section-campaigns font-display">CAMPAIGNS</span> are multi-mission operations.
          ...
          <>
            ...
            <motion.div
              ...
              className={`fixed bottom-0 left-0 right-0 z-50 bg-card border-t-2 rounded-t-2xl max-h-[85vh] overflow-hidden flex flex-col ${
                activeTab === "campaigns"
                  ? "border-section-campaigns"
                  : activeTab === "missions"
                    ? "border-section-missions"
                    : "border-section-command"
              }`}
            >
              {/* Header */}
              <div className="flex items-center justify-between p-4 border-b border-border">
                <div className="flex items-center gap-2">
                  <Filter
                    className={`w-4 h-4 ${activeTab === "campaigns" ? "text-section-campaigns" : activeTab === "missions" ? "text-section-missions" : "text-section-command"}`}
                  />
                  <span
                    className={`font-display text-lg ${activeTab === "campaigns" ? "text-section-campaigns" : activeTab === "missions" ? "text-section-missions" : "text-section-command"}`}
                  >
                    {activeTab === "campaigns"
                      ? "CAMPAIGN FILTERS"
                      : activeTab === "exercises"
                  ...
                  {hasFilters && (
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${activeTab === "campaigns" ? "bg-section-campaigns/20 text-section-campaigns" : activeTab === "missions" ? "bg-section-missions/20 text-section-missions" : "bg-section-command/20 text-section-command"}`}
                    >
                      {
                        [focusFilter, muscleFilter, equipmentFilter, durationFilter, collectionFilter].filter(Boolean)
                          .length
                      }{" "}
                      active
                    </span>
                ...
                {activeTab === "missions" && (
                  <>
                    ...
                    {availableMuscles.length > 0 && (
                      <div>
                        <label className="text-xs text-muted-foreground tracking-wider font-display">
                          PRIMARY MUSCLE
                        </label>
                        <div className="flex flex-wrap gap-2 mt-2">
              ...
              <div className="p-4 border-t border-border bg-card flex gap-3">
                ...
                <button
                  ...
                  className={`flex-1 py-3 px-4 rounded-lg font-display text-sm transition-all ${
                    activeTab === "campaigns"
                      ? "bg-section-campaigns text-white hover:box-glow-campaigns"
                      : activeTab === "missions"
                        ? "bg-section-missions text-white hover:box-glow-missions"
                        : "bg-section-command text-background hover:box-glow-command"
                  }`}
                >
                  APPLY FILTERS
                </button>
                              ...
                              {source === "personal" && mission.created_by === user?.id && (
                                <>
                                  ...
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleDeleteClick(mission.id, mission.code_name, "mission");
                                    }}
            ...
            (exercisesLoading ? (
              ...
              <>
                ...
                <div className="mb-4 p-3 bg-section-command/5 border border-section-command/20 rounded-lg">
                  <p className="text-sm text-muted-foreground leading-relaxed">
                    <span className="text-section-command font-display">EXERCISES</span> are your building blocks.
                    Browse global or create custom moves for your missions.
                  </p>
                </div>
