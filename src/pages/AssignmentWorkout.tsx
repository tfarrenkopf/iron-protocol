...
const AssignmentWorkout = () => {
  ...
  const [cachedAssignment, setCachedAssignment] =
    useState<typeof assignments extends (infer T)[] | undefined ? T : never>();
    ...
    return (
      ...
      >
        ...
        >
          ...
          <h1 className="font-display text-4xl md:text-5xl text-primary text-glow-primary mb-4">{mission.code_name}</h1>
          <p className="text-muted-foreground leading-relaxed mb-8 text-lg">{missionSnapshot.intro_lore}</p>
          <button
    ...
    return (
      ...
      >
        ...
        <motion.div
          ...
          className="max-w-lg text-center relative z-10"
        >
          <h1 className="font-display text-5xl md:text-6xl text-success text-glow-primary mb-4">ORDERS COMPLETE</h1>
          <p className="text-muted-foreground leading-relaxed mb-8 text-lg">{missionSnapshot.outro_lore}</p>
          <button
            onClick={() => setShowLore(null)}
  ...
  const completedSets =
    missionExercises.slice(0, safeExerciseIndex).reduce((acc, e) => acc + e.target_sets, 0) + currentSetIndex;
      ...
      <div className="sticky bottom-0 left-0 right-0 p-4 bg-background/95 backdrop-blur border-t border-border z-20 flex-shrink-0">
        <motion.button
          ...
          className={`w-full py-5 font-display text-2xl rounded-lg transition-all ${
            isCompleting ? "bg-muted text-muted-foreground" : "bg-warning text-warning-foreground hover:opacity-90"
          }`}
        >
          ...
          >
            ...
            >
              ...
              <p className="text-muted-foreground text-sm mb-2">
                You've logged <span className="text-primary font-display">{stats.setsCompleted}</span> sets for your
                handler.
              </p>
              <p className="text-muted-foreground text-sm mb-6">
                Deserting your post means losing your XP, combo streak, and disappointing your handler. Real soldiers
