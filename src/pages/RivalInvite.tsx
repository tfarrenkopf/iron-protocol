...
const RivalInvite = () => {
  ...
  // Auto-accept rivalry if already logged in
  useEffect(() => {
    if (!authLoading && user && rivalProfile && !joined && !autoAccepting) {
      setAutoAccepting(true);
      handleAcceptRival();
    }
  }, [user, authLoading, rivalProfile, joined, autoAccepting]);
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        // Auth state change will trigger auto-accept
      } else {
  ...
  // Auto-accepting state (logged in user)
  if (autoAccepting && !joined) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <Loader2 className="w-12 h-12 animate-spin text-accent mx-auto mb-4" />
          <p className="font-display text-xl text-accent animate-neon-pulse">ESTABLISHING RIVALRY...</p>
        </div>
      </div>
    );
  }
  ...
  // Main invite page with inline auth (for non-logged-in users)
  return (
